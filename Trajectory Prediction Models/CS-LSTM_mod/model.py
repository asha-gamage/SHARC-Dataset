...
class highwayNet(nn.Module): # Create class object 'highwayNet' from super class 'Module'
    ...
    ## Initialization
    def __init__(self, args): # using dunder method, __init__
    ...
    def forward(self,hist,nbrs,masks,lat_enc,lon_enc):
        ...
        hist_enc = self.leaky_relu(self.dyn_emb(hist_enc.view(hist_enc.shape[1],hist_enc.shape[2]))) # Reshaping 'hist_enc'

        ## Forward pass nbrs
        _, (nbrs_enc,_) = self.enc_lstm(self.leaky_relu(self.ip_emb(nbrs)))
        nbrs_enc = nbrs_enc.view(nbrs_enc.shape[1], nbrs_enc.shape[2]) # Reshaping 'nbrs_enc'

        ## Masked scatter
        soc_enc = torch.zeros_like(masks).float()
        soc_enc = soc_enc.masked_scatter_(masks.bool(), nbrs_enc)#.bool()
        ...
        soc_enc = soc_enc.view(-1,self.soc_embedding_size) # Reshaping 'soc_enc'
        ...
        enc = torch.cat((soc_enc,hist_enc),1) #concatenate along dimension 1
    ...
    def decode(self,enc):
        enc = enc.repeat(self.out_length, 1, 1) #repeat() function is used to repeat the individual elements of an array a specified number of times.
                                                # Syntax: numpy.repeat(arr, repeats, axis=None)
...
#     'encoder_size': 32,
#     'decoder_size': 32,
#     'in_length': 30,     # t_h (history length)
#     'out_length': 50,    # t_f (future length)
#     'grid_size': (13,3),
#     'soc_conv_depth': 64,
#     'conv_3x1_depth': 32,
...
# # ===== Create dummy inputs =====
# batch_size = 1
# seq_len = args['in_length']
# feature_dim = 2  # always 2, matches ip_emb = Linear(2, input_embedding_size)
# nbr_dim = 2      # each neighbor has 2 features
# num_lat_classes = args['num_lat_classes']
# num_lon_classes = args['num_lon_classes']
# grid_x, grid_y = args['grid_size']
# enc_size = args['encoder_size']

# # hist: (seq_len, batch_size, feature_dim)
# hist = torch.randn(seq_len, batch_size, feature_dim)

# # nbrs: (max_neighbors, batch_size, feature_dim)
# max_neighbors = grid_x * grid_y
# nbrs = torch.randn(max_neighbors, batch_size, nbr_dim)

# # masks: (batch_size, grid_y, grid_x, encoder_size)
# # masks = torch.ones(batch_size, grid_y, grid_x, enc_size)
# masks = torch.zeros(batch_size, grid_y, grid_x, enc_size, dtype=torch.bool)

# # Maneuver encodings
# lat_enc = torch.zeros(batch_size, num_lat_classes)
# lon_enc = torch.zeros(batch_size, num_lon_classes)

# # ===== Profile FLOPs and parameters =====
# flops, params = profile(model, inputs=(hist, nbrs, masks, lat_enc, lon_enc))
