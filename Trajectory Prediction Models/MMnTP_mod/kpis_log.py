...
def MMnTP_kpis(p, kpi_input_dict, traj_min, traj_max, figure_name, save_path=None):
    '''
    1. NLL (TBD)
    2. error based 
    Miss Rate (MR):  The number of scenarios where none of the forecasted trajectories are within 2.0 meters of ground truth according to endpoint error.
    ...
    # ---------------------------------------------------------
    # Save results dictionary to .pt file if path is provided
    # ---------------------------------------------------------
    if save_path is not None:
        ...
        print(f"KPI results saved to: {save_path}")
...
def calc_minFDE(p, traj_pred, mode_prob, traj_gt, mr_thr = 2):
    #TODO multimodal version of this metric need to be updated similar to minRMSE
