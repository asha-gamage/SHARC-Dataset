...
def train_model_dict(p, prev_best_model = None, prev_itr = 1):
    ...
    print("TRAIN dataset size:", len(tr_dataset)) # 12/06/26
    ...
    print("VAL dataset size:", len(val_dataset)) # 12/06/26
    ...
    print("TEST dataset size:", len(te_dataset)) # 12/06/26
...
if __name__ == '__main__':
    ...
    #                              seperate_deploy_dataset='m40_deploy.yaml') # commented on 30/04/26
    p = params.ParametersHandler('MMnTP.yaml', 'combi.yaml', './config') # changed on 30/04/26
    
    # p.hyperparams['experiment']['group'] = 'povl_sm'
    p.hyperparams['experiment']['group'] = 'merging' #changed 30/04/26
    p.hyperparams['experiment']['debug_mode'] = False  #True changed 30/04/26
