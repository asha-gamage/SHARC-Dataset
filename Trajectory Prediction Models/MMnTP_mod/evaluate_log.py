import os
import re # added 21/05/26
import logging
from time import time
from datetime import datetime # added on 21/05/26
import numpy as np 
import pickle
import random
...
import TPMs 
import params
import top_functions_log #

import torch.multiprocessing
torch.multiprocessing.set_sharing_strategy('file_system')
import pdb 
import kpis_log #updated on 21/05/26
import matplotlib.colors as mcolors
import TPMs
def test_model_dict(p):
    # Set Random Seeds:
    # if False:#torch.cuda.is_available() and p.CUDA:  #changed 30/04/26
    ...
    if p.model_dictionary['hyperparams']['probabilistic output']:
        traj_loss_func = kpis_log.NLL_loss
    else:
    ...
    # Create a path for saving the resulting metrics:
    ...
    #     save_dir,
    #     f"MMnTP_kpis_{timestamp}.pt"
    # )
    
    exp_name = os.path.basename(experiment_file)

    # remove anything after first space or underscore timestamp patterns
    exp_name = re.split(r"\s", exp_name)[0]

    kpi_filename = f"{exp_name}_kpis.pt"

    save_path = os.path.join(save_dir, kpi_filename)
    ...
    kpi_dict = top_functions_log.eval_top_func(p, model_eval_func, model_kpi_func, model, (traj_loss_func, man_loss_func), te_dataset, device, save_path=save_path)# updated 21/05/26
...
if __name__ == '__main__':
    ...
    #                               seperate_test_dataset='m40_test.yaml',
    #                               seperate_deploy_dataset='m40_deploy.yaml') commented 30/04/26
    
    p = params.ParametersHandler('MMnTP.yaml', 'sim_train.yaml', './config', seperate_test_dataset='ngsim_test.yaml') # POVL_SM.yaml
                                #   seperate_test_dataset='m40_test.yaml',
                                #   seperate_deploy_dataset='m40_deploy.yaml')
    
    # experiment_file = 'experiments/POVL_SM_m40_train_2024-02-04 12:56:38.408576' # changed 30/04/26
    experiment_file = 'experiments/MMnTP_simData_2026-06-17 10:12:52.065105' # changed 30/04/26
    p.import_experiment(experiment_file)
    p.hyperparams['experiment']['debug_mode'] = False #True changed to 'false' on
