...
# import loader2_getDistance as lo
import loader2_hybrid as lo # updated to 'loader2_270525' from 'loader2'
import torch.optim as optim
from torch.optim.lr_scheduler import ExponentialLR
from tqdm import tqdm
import os
from evaluate5f_hybrid import Evaluate
from config import *
...
def save_checkpoint(epoch,
    ...
    checkpoint = {
        ...
        'gdEncoder_state_dict': gdEncoder.state_dict(),
        'generator_state_dict': generator.state_dict(),

        'optimizer_gd_state_dict': optimizer_gd.state_dict(),
        'optimizer_g_state_dict': optimizer_g.state_dict(),

        'scheduler_gd_state_dict': scheduler_gd.state_dict(),
        'scheduler_g_state_dict': scheduler_g.state_dict()
    }

    t.save(checkpoint, os.path.join(path, 'checkpoint_latest_3070.pth'))
...
def load_checkpoint(path,
    ...
    checkpoint_file = os.path.join(path, 'checkpoint_latest_3070.pth')

    if not os.path.exists(checkpoint_file):
        print("No checkpoint found. Starting from scratch.")
        return 0

    checkpoint = t.load(checkpoint_file, map_location=device)

    gdEncoder.load_state_dict(checkpoint['gdEncoder_state_dict'])
    generator.load_state_dict(checkpoint['generator_state_dict'])
    optimizer_gd.load_state_dict(checkpoint['optimizer_gd_state_dict'])
    optimizer_g.load_state_dict(checkpoint['optimizer_g_state_dict'])
    scheduler_gd.load_state_dict(checkpoint['scheduler_gd_state_dict'])
...
def main():
    ...
    if t.cuda.is_available(): # added on 190525
        print('Running on GPU')
        
    f = open('Training on combiData MSE Log.txt', 'w')  # Open a text file to save the calculated RMSE values at each range setting
    ...
    if dataset == "ngsim":
        if args['lon_length'] == 3:
            # t1 = lo.NgsimDataset('/home/asha/stdan-master/pythonScripts/dataset/combiTrainSet.mat')
            t1 = lo.NgsimDataset('dataset/STDAN Retrain_Combi 30_70/trainTrajSet.mat', 'dataset/STDAN Retrain_Combi 30_70/trainTracks.mat')
        else:
            t1 = lo.NgsimDataset('C:/Users/gamage_a/Documents/Python/stdan-master/data/5feature/TrainSet.mat')
        trainDataloader = DataLoader(t1, batch_size=args['batch_size'], shuffle=True, num_workers=args['num_worker'],
                                     collate_fn=t1.collate_fn) # Set shuffle to 'False' to find the vehIDs with non assigned lanes and class.
    else:
    ...
    # for epoch in range(args['epoch']):
        ...
        save_checkpoint(epoch, gdEncoder, generator, optimizer_gd, optimizer_g, scheduler_gd, scheduler_g,
                    args['path'])
...
def save_model(name, gdEncoder, generator, path):
    ...
    if not os.path.exists(l_path):
        os.makedirs(l_path)
    t.save(gdEncoder.state_dict(), l_path + '/epoch' + name + '_gd_combi30_70.tar')
    t.save(generator.state_dict(), l_path + '/epoch' + name + '_g_combi30_70.tar')
