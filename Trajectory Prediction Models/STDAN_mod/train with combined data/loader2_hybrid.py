...
class NgsimDataset(Dataset):
    # Added reading two types of matfiles: traj saved in -v7.3 (HDF5 format) and 'tracks' saved in -v7. 
    def __init__(self, mat_file_1, mat_file_2, t_h=30, t_f=50, d_s=2, enc_size=64, grid_size=(13, 3)): 
        # self.D = scp.loadmat(mat_file_1)['traj']
        # f = h5py.File(mat_file_1, 'r')
        # self.D = np.array(f['traj'])
        ...
        # self.D = self.D.T        
        self.T = scp.loadmat(mat_file_2)['combiTracks']
        self.t_h = t_h  # 
        self.t_f = t_f  # 
        self.d_s = d_s  # skip
    ...
    def __getitem__(self, idx):
        ...
        # RdAng = self.D[idx, 9] # Added
        # t = self.D[idx, 2]  # frame
        # grid = self.D[idx, 12:]  #  grid id
        ...
        hist = self.getHistory(vehId, t, vehId, dsId, RdAng)  # 'RdAng' added
        refdistance = np.zeros_like(hist[:, 0])
        refdistance = refdistance.reshape(len(refdistance), 1)
        fut = self.getFuture(vehId, t, dsId, RdAng)  # 'RdAng' added
        ...
        for i in grid:
            nbrsdis = self.getHistory(i.astype(int), t, vehId, dsId, RdAng) # 'RdAng' added
        ...
        # lon_enc[int(self.D[idx, 11] - 1)] = 1 # Updated to 11 from 10
        lon_enc[int(row[11] - 1)] = 1
        lat_enc = np.zeros([args['lat_length']])
        # lat_enc[int(self.D[idx, 10] - 1)] = 1 # Updated to 10 from 9
        lat_enc[int(row[10] - 1)] = 1

        return hist, fut, neighbors, lat_enc, lon_enc, va, neighborsva, lane, neighborslane, refdistance, neighborsdistance, cclass, neighborsclass, vehId
    ...
    def getLane(self, vehId, t, refVehId, dsId):
        if vehId == 0:
            ...
            matches = np.where(vehTrack[:, 0] == t)[0]

            if matches.size == 0 or vehTrack.size == 0:
                return np.empty([0, 1])
            if matches.size > 1:
                print(f"Warning: Expected one match for t={t}, found {matches}")
                print(f"dsID = ", dsId)
                print(f"vehID = ", vehId)

            enpt = matches[0] + 1
            stpt = np.maximum(0, enpt - 1 - self.t_h)
    ...
    def getHistory(self, vehId, t, refVehId, dsId, RdAng):
        if vehId == 0:
            ...
            # convert coordinates to the target vehicle frame
            refPosCon = self.cordConv(refPos[1], refPos[0], RdAng)
            ...
            # convert coordinates to the target vehicle frame
            histCon = self.cordConv(histOri[:,1], histOri[:,0], RdAng)
            histCon = np.asarray(histCon).transpose()
            refPosCon = np.asarray(refPosCon)
            hist = np.asarray(histCon)

            # For undetected timeframes (zero coordinates for x, y),
            # set the hist to (0,0).
    ...
    def getFuture(self, vehId, t, dsId, RdAng):
        ...
        futOri = vehTrack[stpt:enpt:self.d_s, 1:3]
        futCon = self.cordConv(futOri[:, 1], futOri[:, 0], RdAng)
        futCon = np.asarray(futCon).transpose()

        fut = futCon[:, :] - refPosCon
        fut[:, [0, 1]] = fut[:, [1, 1-1]]  # equivalent swap [0,1] <- [1,0]
        return fut
    ...
    def collate_fn(self, samples):
        ...
        nbr_batch_size = 0
        for _, _, nbrs, _, _, _, _, _, _, _, _, _, _,vehId in samples:
        ...
        for sampleId, (hist, fut, nbrs, lat_enc, lon_enc, va, neighborsva, lane, neighborslane, refdistance,
                       neighborsdistance, cclass, neighborsclass, vehId) in enumerate(samples):
