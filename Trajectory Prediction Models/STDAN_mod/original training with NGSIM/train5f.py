...
def main():
    ...
    if dataset == "ngsim":
        if args['lon_length'] == 3:
            # t1 = lo.NgsimDataset('C:/Users/gamage_a/Documents/Python/stdan-master/data/dataset_t_v_t/TrainSet.mat')
            t1 = lo.NgsimDataset('dataset/ngsim_TrainSet.mat')
        else:
...
def save_model(name, gdEncoder, generator, path):
    ...
    if not os.path.exists(l_path):
        os.makedirs(l_path)
    t.save(gdEncoder.state_dict(), l_path + '/epoch' + name + '_gd_ngsim.tar')
    t.save(generator.state_dict(), l_path + '/epoch' + name + '_g_ngsim.tar')
