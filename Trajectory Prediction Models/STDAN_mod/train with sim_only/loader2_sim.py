...
class NgsimDataset(Dataset):
    ...
    def __init__(self, mat_file, t_h=30, t_f=50, d_s=2, enc_size=64, grid_size=(13, 3)):
        ...
        self.T = scp.loadmat(mat_file)['simTracks']
        self.t_h = t_h  # 
        self.t_f = t_f  # 
        self.d_s = d_s  # skip
    ...
    def getHistory(self, vehId, t, refVehId, dsId, RdAng): # 27/05/25: function updated by adding 'RdAng'
        if vehId == 0:
            return np.empty([0, 2])
        else:
            if self.T.shape[1] <= vehId - 1:
            ...
            if vehTrack.size == 0 or np.argwhere(vehTrack[:, 0] == t).size == 0:
                ...
                if indices.size == 1:
                    stpt = np.maximum(0, indices.item() - self.t_h)
                else:
                # Handle the case where there are 0 or >1 matches
                    print(f"Warning: Expected one match for t={t}, found {indices.size}")
                    stpt = 0  # or handle appropriately
    ...
    def getFuture(self, vehId, t, dsId, RdAng): # 27/05/25: function updated by adding 'RdAng'
        vehTrack = self.T[dsId - 1][vehId - 1].transpose()  
