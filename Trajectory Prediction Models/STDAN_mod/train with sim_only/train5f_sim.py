...
import loader2_sim as lo # updated from 060625 on 240526 for combined data use
import torch.optim as optim
from torch.optim.lr_scheduler import ExponentialLR
from tqdm import tqdm
import os
from evaluate5f_sim import Evaluate
from config import *
...
def main():
    ...
    f = open('Training on simOnlyData MSE Log.txt', 'w')  # Open a text file to save the calculated RMSE values at each range setting
    ...
    if dataset == "ngsim":
        if args['lon_length'] == 3:
            ...
            t1 = lo.NgsimDataset('dataset/STDAN Retrain Sim/simTrainSet.mat')
        else:
...
def save_model(name, gdEncoder, generator, path):
    ...
    if not os.path.exists(l_path):
        os.makedirs(l_path)
    t.save(gdEncoder.state_dict(), l_path + '/epoch' + name + '_gd_simOnly.tar')
    t.save(generator.state_dict(), l_path + '/epoch' + name + '_g_simOnly.tar')
