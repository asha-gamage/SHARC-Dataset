%%This script geneates an equivalent .mat file for each simulated LC
...
% folderPath = 'C:\Users\gamage_a\Documents\CM_Curves\SimOutput\WMGL241\20250131\textFiles4STDAN\'; 
folderPath = 'C:\Users\gamage_a\Documents\CM_Curves\matlabScripts\set85\textFiles4STDAN\';
% folderPath = '..\SimOutput\WMGL241\20250131\textFiles4STDAN\';

% Get a list of all .mat files in the folder
txtFiles = dir(fullfile(folderPath, '*.txt'));
N = length(txtFiles); 
groupedTraj = cell(5,1);
...
% ===================================================
    ...
    for k = 1:length(traj(:,1))
        ...
        if vehtraj(ub,8) > vehtraj(ind,8) || vehtraj(ind,8) > vehtraj(lb,8)% future lane Id > current lane Id OR current lane Id > past lane Id
            traj(k,11) = 3; % Right Lane-change and adds it to a new column_8
        elseif vehtraj(ub,8) < vehtraj(ind,8) || vehtraj(ind,8) < vehtraj(lb,8)
            traj(k,11) = 2; % Left Lane-change
        else
            traj(k,11) = 1; % Lane Keep
        end
        ...
        if ub==ind || lb ==ind % If current index is the start OR the end of the recording....
            traj(k,12) = 1;% longitudinal maneuver is categorised as 'Normal speed'
        else
        ...
        if ~isempty(frameL)
            for l = 1:size(frameL,1)
                ...
                if abs(y) <90 % 90feet distance boundary
                    ...
                    traj(k,12+gridInd) = frameL(l,2); % Adds the new columns to the matrix after column 8, which is the longitudinal maneuver
                end
            end
        end
        for l = 1:size(frameEgo,1)
            ...
            if abs(y) <90 && y~=0
                ...
                traj(k,12+gridInd) = frameEgo(l,2);
            end
        end
        if ~isempty(frameR)
    ...
    folder1 = 'C:\Users\gamage_a\Documents\CM_Curves\matlabScripts\set85\matFiles4STDAN_Combi';
    folder2 = 'C:\Users\gamage_a\Documents\CM_Curves\matlabScripts\set85\matFiles4STDAN_Sim';
...
for h = 1:5
    ...
    for v = 1:length(vehIds)
        ...
        tracks_CM {h, vehIds(v)} = vehTrack(:, [3:9, 11:12])';
    end
end
savePath_1 = 'C:\Users\gamage_a\Documents\CM_Curves\matlabScripts\set85\matFiles4STDAN_Combi\sim_tracks';
savePath_2 = 'C:\Users\gamage_a\Documents\CM_Curves\matlabScripts\set85\matFiles4STDAN_Sim\sim_tracks';
